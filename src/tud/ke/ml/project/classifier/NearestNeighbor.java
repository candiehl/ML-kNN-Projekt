...
public class NearestNeighbor extends ANearestNeighbor {
	...
	protected List<Pair<List<Object>, Double>> getNearest(List<Object> testdata) {
		...
		case 0: { // Manhattan distance
			for(List<Object> trainInstance : traindata) {
				nearest.add(new Pair<List<Object>, Double>(trainInstance, determineManhattanDistance(trainInstance, testdata)));
			}
			break;
		}
		case 1: { // Euclidean distance
			for(List<Object> trainInstance : traindata) {
				nearest.add(new Pair<List<Object>, Double>(trainInstance, determineEuclideanDistance(trainInstance, testdata)));
			}
			break;
		}
		// default: do nothing
		...
		Collections.sort(nearest, new Comparator<Pair<List<Object>, Double>>() {
			@Override
			public int compare(Pair<List<Object>, Double> element1, Pair<List<Object>, Double> element2) {
	...
	private List<Pair<List<Object>, Double>> breakNearestTie(List<Pair<List<Object>, Double>> nearest, int k) {
		return nearest.subList(0, this.getkNearest()); // TODO: implement proper selection strategy
	}
	@Override
